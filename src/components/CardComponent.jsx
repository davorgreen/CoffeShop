...
function CardComponent({ item, label, handleOpen, handleClose, modalOpen, handleRemoveFavorite, index }) {
    ...
    return (
        <div className="gap-4 mt-10 p-5 md:flex-row justify-center items-center bg-green-200 shadow-2xl rounded-lg overflow-hidden m-4 relative">
            ...
            {/* Right side */}
            <div className="flex flex-col p-4 gap-2 justify-between text-center items-center h-full" style={{ height: '330px' }}>
                <div className=" flex flex-col gap-4">
                    <div className="text-xl text-green-800 font-bold">{name}</div>
                    <div className="text-gray-700 font-bold">{shortDescription}</div>
                </div>
